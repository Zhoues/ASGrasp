...
for k, v in os.environ.items():
    if k.startswith("QT_") and "cv2" in v:
        del os.environ[k]
from .utils.utils import plot_gripper_pro_max, batch_rgbdxyz_2_rgbxy_depth, get_batch_key_points, batch_key_points_2_tuple, framexy_depth_2_xyz, batch_framexy_depth_2_xyz, center_depth, key_point_2_rotation, batch_center_depth, batch_framexy_depth_2_xyz, batch_key_point_2_rotation, plot_gripper_pro_max_robotiq
...
class Grasp():
    ...
    def to_open3d_geometry(self, color=None):
        ...
        **Ouput:**
        ...
        '''
        # return plot_gripper_pro_max_robotiq(self.translation, self.rotation_matrix, self.width, self.depth, score = self.score, color = color)

        return plot_gripper_pro_max(self.translation, self.rotation_matrix, self.width, 0, score=self.score,
                                color=color)
...
class GraspGroup():
    ...
    def sort_by_height(self, reverse = False):
        '''
        **Input:**
        ...
        **Output:**
        ...
        - no output but sort the grasp group.
        '''
        score = self.grasp_group_array[:,15]
        index = np.argsort(score)
        if not reverse:
            index = index[::-1]
        self.grasp_group_array = self.grasp_group_array[index]
